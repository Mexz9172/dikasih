...
haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Selamat Tinggal *@${num.split("@")[0]}* Di Group *${metadata.subject}*`})
...
//autostatus view
        haikal.ev.on('messages.upsert', async chatUpdate => {
            ...
            if (mek.key && mek.key.remoteJid === 'status@broadcast') {
            	await haikal.readMessages([mek.key]) }
            }
    })
//=================================================//
...
haikal.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
let type = await haikal.getFile(path, true)
...
let m
try {
m = await haikal.sendMessage(jid, message, { ...opt, ...options })
} catch (e) {
console.error(e)
m = null
} finally {
if (!m) m = await haikal.sendMessage(jid, { ...message, [mtype]: file }, { ...opt, ...options })
return m
}
}
//=================================================//
...
connectToWhatsApp();
  ...
  haikal.sendMessage('6282287486762' + "@s.whatsapp.net", { text: `> DHA BOTXZ Aktif✅` });
}
